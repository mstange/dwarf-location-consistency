...
pub struct ResDwarf<R: gimli::Reader> {
    ...
    fn parse(sections: Arc<gimli::Dwarf<R>>) -> Result<Self, Error> {
        ...
        while let Some(header) = units.next()? {
            ...
            let mut lang = None;
            {
                ...
                // discontiguous CUs.
                let aranges_index = match ranges.ranges_offset {
                    Some(_) => None,
                    None => aranges.binary_search_by_key(&offset, |x| x.0).ok(),
                };
                if let Some(mut i) = aranges_index {
...
pub struct RangeAttributes<R: gimli::Reader> {
    low_pc: Option<u64>,
    high_pc: Option<u64>,
    size: Option<u64>,
